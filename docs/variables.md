...
| `adempiere_profile` | vars.yml | **O** | `all` | Profile passed to `start-all.sh` and `health-check.sh`. Controls which subset of services is started. `all` starts every service defined in the stack. Available profiles: `vue`, `zk`, `auth`, `cache`, `report`, `scheduler`, `storage`, `all`. See [adempiere-ui-gateway](https://github.com/Systemhaus-Westfalia/adempiere-ui-gateway) for the full profile list. | `all`, `zk`, `vue` |
